...
class CoCa(nn.Module):
    ...
    def _compute_image_multiscale(self, image_embs: torch.Tensor):
        feats = {"scale_1": image_embs}
        if self.img_decoder is None:
            return feats

        x = image_embs.transpose(1, 2)  # [B, D, L] 
        for name, pool in self.img_decoder.items():
            y = pool(x).transpose(1, 2)  # [B, L/s, D]
            feats[name] = y
        return feats

    def _encode_image(self, image, tops=None, lefts=None, crop_size=None, normalize: bool = True):
        image_latent, tokens_embs = self.visual(image, tops, lefts, crop_size)
    ...
    def forward(
            ...
            tops: Optional[torch.Tensor] = None,
            lefts: Optional[torch.Tensor] = None,
            crop_size: Optional[torch.Tensor] = None,
            output_labels: bool = True,
    ):
        if image_latent is None or image_embs is None:
            # image_latent=[B,D] 
            # image_latent (128, 1, 512), image_embs (128, 256, 512)
            if crop_size is not None:
                image_latent, image_embs = self._encode_image(image, tops, lefts, crop_size)
            else:
        ...
        # image_embs: [B, L, D]
        zero_tok = torch.zeros_like(image_embs[:, :1, :])
        image_inputs_ar = torch.cat([zero_tok, image_embs[:, :-1, :]], dim=1)  # teacher forcing

        logits, multi_scale_text_blocks = self.text_decoder(image_embs, token_embs)
