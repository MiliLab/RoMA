...
class PatchSelect(nn.Module):
    """
    https://arxiv.org/abs/2406.11933
    PatchSelect with Auto HOG Pool Calculation
    """

    @staticmethod
    def _calculate_optimal_hog_pool(grid_size: Tuple[int, int], image_size: int = 224, preferred_hog_pool: int = 8) -> int:
        Gh, Gw = grid_size
        grid_dim = Gh
        
        for hog_pool in range(preferred_hog_pool, image_size // grid_dim + 1):
            if (image_size // hog_pool) % grid_dim == 0:
                return hog_pool
        
        return image_size // grid_dim
    ...
    def __init__(
        ...
        image_size: int = 224,                        
        hog_gaussian_window: int = 16,                
        auto_hog_pool: bool = True,                   
        manual_hog_pool: Optional[int] = None,       
        preferred_hog_pool: int = 8,                  
    ):
        super().__init__()
        
        if manual_hog_pool is not None:
            hog_pool = manual_hog_pool
        elif auto_hog_pool and grid_size is not None:
            hog_pool = self._calculate_optimal_hog_pool(
                grid_size=grid_size,
                image_size=image_size,
                preferred_hog_pool=preferred_hog_pool
            )
        else:
            hog_pool = preferred_hog_pool
        
        self.hog = HOGLayerC(
            pool=hog_pool,
            gaussian_window=hog_gaussian_window,
            grid_size=grid_size,
        )
        ...
        self.hog_pool = hog_pool
        self.hog_gaussian_window = hog_gaussian_window
    ...
    def forward(
        ...
        ids_unselected = all_indices[unselected_mask].view(
            p_x.shape[0], p_x.shape[1] - ids_selected.shape[1]
        )  # (B, L - S)

        select_token = torch.gather(
            x_patches, dim=1, index=ids_selected.unsqueeze(-1).repeat(1, 1, D)
        )  # (B, S, D)
        unselect_token = torch.gather(
            x_patches, dim=1, index=ids_unselected.unsqueeze(-1).repeat(1, 1, D)
        )  # (B, L-S, D)
        ...
        if self.exclude_first_token:
            out = torch.cat([cls_tokens, select_token], dim=1)  #
        else:
    ...
    def get_config(self) -> dict:
        return {
            ...
            "image_size": self.image_size,
            "hog_pool": self.hog_pool,
            "hog_gaussian_window": self.hog_gaussian_window,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # --------------------------------------
        self.mask_ratio = 1.0 - float(patch_select)
        
        if patch_select > 0:
            self.patch_keep = PatchSelect(
                exclude_first_token=exclude_first_token,
                grid_size=self.grid_size          
            )
        elif patch_dropout > 0:
            self.patch_keep = PatchDropout(prob=patch_dropout, exclude_first_token=exclude_first_token)
        else:
            self.patch_keep = nn.Identity()
            
        self.patch_dropout_prob = patch_dropout
        ...
        self.ln_post = norm_layer(pool_dim)  # if attentional_pool: width=768, output_dim=512
    ...
    def _embeds(self, x:torch.Tensor) -> torch.Tensor:
        ...
        elif self.patch_dropout_prob > 0:
            pass
...
class MultimodalTransformer(Transformer):
    ...
    def forward(self, image_embs, text_embs):
        ...
        multi_scale_text_blocks = {}
        kernel_list = getattr(self, "kernel_list", [2,4])  
        for k in kernel_list:
            if k <= 1 or out.shape[1] // k < 1:
                continue
            ds = F.interpolate(out.permute(0, 2, 1), scale_factor=1.0 / k, mode="linear", align_corners=False)
            ds = ds.permute(0, 2, 1)
            multi_scale_text_blocks[f"scale_{k}"] = ds

        if self.text_projection is not None:
            out = out @ self.text_projection
            for scale, features in multi_scale_text_blocks.items():
                multi_scale_text_blocks[scale] = features @ self.text_projection

        return out, multi_scale_text_blocks
...
def _unpatchify(patches: torch.Tensor, H: int, W: int, P: int, C: int = 3):
    # patches: [B, L, C*P*P], where L = (H/P)*(W/P)
    B, L, D = patches.shape
    h, w = H // P, W // P
    x = patches.view(B, h, w, C, P, P)         # B, h, w, C, P, P
